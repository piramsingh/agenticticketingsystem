...
export class TicketAgentPanel implements vscode.WebviewViewProvider {
  ...
  resolveWebviewView(
    ...
    webviewView.webview.onDidReceiveMessage(async (msg) => {
      switch (msg.command) {
        case 'createTicket': await this._createTicket(msg.text); break;
        case 'listTickets':  await this._listTickets(); break;
        case 'scanCodebase': await this._scanCodebase(); break;
        case 'openFile':     await this._openFile(msg.file, msg.line); break;
  ...
  private async _openFile(relativePath: string, line: number) {
  ...
  private _request(path: string, method = 'GET', body?: object): Promise<any> {
    return new Promise((resolve, reject) => {
      const url = new URL(path, getConfig().backendUrl);
  ...
  private async _createTicket(text: string) {
